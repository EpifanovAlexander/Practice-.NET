	...
	{	
		...
		public bool RemoveRewardById(int id)
        {
			...
			foreach (UsersModel user in storageUsers)
            {
				...
				user.RewardsIsCheck.Remove(user.RewardsIsCheck[storageRewards.IndexOf(ReturnRewardById(id))]);
			}
		...
		public bool RewardUser(int userId)
        {
			UsersModel user = ReturnUserById(userId);
            ...
            {
				if (user.RewardsIsCheck[i]) user.Rewards.Add(storageRewards[i]);
			}
			return true;
		}
